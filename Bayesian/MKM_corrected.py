...
# # Microkinetic Model for CO Oxidation (Pd), (PdO) and (PdO2) + O2 ads - Bayesian Inference (pymc2) + Hansen like - "Run 1- Truncated normal - Case 5 (ABCDE)"
...
#   SCO2v = Sv = SO2v = SO = SCOO = 0
    ...
    dE[5] = -2.01                 # 2nd CO adsorption (-0.65)
    dE[6] =  0.02                 # 2nd CO oxidation using far O from Pd
    dE[7] =  0.37               # 2nd CO2 desorption
    dE[8] = -0.76              # O migration from Pd-far to near to form Pd and regenerate the cycle
...
#     dSS = 0.001
    ...
    Ea[4] =  0                     # O2 dissociation barrier
    Ea[6] =  0.60                 # 2nd CO Oxidation barrier (using far-off O)
    Ea[8] =  0.30       # O migration from Pd-far to near to form Pd and regenerate the cycle
    ...
    Ea[15] = 0                    # CO oxidation at 2O*
    Ea[18] = 1.44                 # O2 dissociation on * to give 2O*
...
#     for t in theta:
#         print (t)
    return (rates[1]+rates[6]+rates[9]+rates[15])
...
# Getting the reaction order for PCO
...
PCO_range = np.linspace(0.03,0.05,gridpoints)
for i,PCO in enumerate(PCO_range):
    rate_PCO[i]=print_output(theta0,T,[PCO,0.20])
    #Make a list/tuple for all PCO and rate_PCO


# Getting the reaction order for PO2
...
PO2_range = np.linspace(0.15,0.25,gridpoints)
for i,PO2 in enumerate(PO2_range):
    rate_PO2[i]=print_output(theta0,T,[0.02,PO2])
    #Make a list/tuple for all PCO and rate_PCO
# rate_PO2
...
plt.subplot(121)
plt.plot(PCO_range, rate_PCO, 'b', label='PCO')
plt.xlabel('PCO')
plt.ylabel('rate_CO2_formation')
plt.subplot(122)
plt.plot(PO2_range, rate_PO2, 'g', label='O2')
plt.ylabel('rate_CO2_formation')
plt.xlabel('PO2')
plt.show()
...
order_O2=LR_O2.coef_
# LR.intercept_


# Apparent barrier Calculation
...
T_range = np.linspace(473,483,gridpoints)
for i,T in enumerate(T_range):
    rate_app[i]=print_output(theta0,T,[0.02,0.20])
    #Make a list/tuple for all PCO and rate_PCO
rate_app
#visualize the plot and make a linear regression plot
...
print('Reaction order of CO: %.4f ' % order_CO[0])
print('Reaction order of O2: %.4f ' % order_O2[0])
print('Apparent activation Barrier: %.2f eV' % apparent_barrier[0])
...
# # Bayesian Inference for getting the probability distribution
...
# # Reaction orders and apparent barrier
